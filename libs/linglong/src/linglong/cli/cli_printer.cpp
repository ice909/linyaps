...
void CLIPrinter::printSearchResult(const std::vector<std::pair<std::string, api::types::v1::PackageInfoV2>> &list)
{
    if (list.empty()) {
    ...
    for (const auto &item : list) {
        auto simpleDescription = QString::fromStdString(item.second.description.value_or("")).simplified();
        auto simpleDescriptionWStr = simpleDescription.toStdWString();
        auto simpleDescriptionWcswidth = wcswidth(simpleDescriptionWStr.c_str(), -1);
        if (simpleDescriptionWcswidth > 56) {
            simpleDescriptionWStr = subwstr(simpleDescriptionWStr, 53) + L"...";
        ...
        auto name = QString::fromStdString(item.second.name).simplified();
        auto nameWStr = name.toStdWString();
        auto nameWcswidth = wcswidth(nameWStr.c_str(), -1);
        if (nameWcswidth > 33) {
            nameWStr = subwstr(nameWStr, 29) + L"...";
        ...
        auto nameStr = name.toStdString();
        auto nameOffset = nameStr.size() - nameWcswidth;
        std::cout << std::setw(43) << item.second.id + " " << std::setw(33 + nameOffset) << nameStr + " "
                  << std::setw(16) << item.second.version + " " << std::setw(16) << item.second.channel + " "
                  << std::setw(12) << item.second.packageInfoV2Module + " " << std::setw(10) << item.first + " "
                  << simpleDescription.toStdString() << std::endl;
    }
}
