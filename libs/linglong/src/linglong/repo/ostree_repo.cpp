                      ...
                      const std::string &module) noexcept
{
    ...
    const auto defaultRepo = getDefaultRepo(this->cfg);
    qDebug() << "\033[33m" << "pull "<< refString.c_str() << "from " <<  QString::fromStdString(!reference.repo.name.empty() ?reference.repo.alias.value_or(reference.repo.name): defaultRepo.name) << "\033[0m";
    auto status =
      ostree_repo_pull_with_options(this->ostreeRepo.get(),
                                    !reference.repo.name.empty() ?reference.repo.alias.value_or(reference.repo.name).c_str(): defaultRepo.name.c_str(),
                                    pull_options,
                                    progress,
                                    cancellable,
                                    &gErr);
    ...
    item.commit = commit;
    item.info = *info;
    item.repo = !reference.repo.name.empty() ?reference.repo.alias.value_or(reference.repo.name).c_str(): defaultRepo.alias.value_or(defaultRepo.name).c_str();
                           ...
                           const std::string &module) const noexcept
{
    ...
    for (const auto &repo : repoConfig.repos) {
        qDebug() << "\033[33m" << "list Remote repo : " << repo.alias.value_or(repo.name).c_str() << "\033[0m";
        ...
        for (auto record : *list) {
            ...
            qDebug() << "\033[36m" << record.channel.c_str() << fuzzy.channel.value_or("") << "\033[0m";
            if (fuzzy.channel && fuzzy.channel->toStdString() != record.channel) {
                continue;
            }
            qDebug() << "\033[36m" << record.id.c_str() << "fuzzy id: " << fuzzy.id << "\033[0m";
            ...
            qDebug() << "\033[36m" << "find module: "<< record.packageInfoV2Module.c_str() << " module: " << module.c_str() << " in : " << QString::fromStdString(repo.alias.value_or(repo.name)) << "\033[0m";
            if (module == "binary") {
...
utils::error::Result<std::vector<api::types::v1::PackageInfoV2>>
OSTreeRepo::listRemote(const package::FuzzyReference &fuzzyRef, const linglong::api::types::v1::Repo &repo) const noexcept
{
